<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
        ...
        <head>
            ...
            <script defer src="/static/js/board/list.js"></script>
        </head>
        ...
        <body>
            <%@ include file="/WEB-INF/views/common/header.jsp" %>
                ...
                <main>
                    <h1 class="content-h1">게시글 목록</h1>

                    <form action="/board/list" method="get">
                        <%-- <select name="sort">
                            <option value="" <c:if test="${sort == null || sort == ''}">selected</c:if>>전체보기</option>
                            <option value="viewDesc" <c:if test="${sort == 'viewDesc'}">selected</c:if>>조회순</option>
                            <option value="titleAsc" <c:if test="${sort == 'titleAsc'}">selected</c:if>>이름순</option>
                            <option value="dateDesc" <c:if test="${sort == 'dateDesc'}">selected</c:if>>최신순</option>
                            <option value="dateAsc" <c:if test="${sort == 'dateAsc'}">selected</c:if>>오래된순</option>
                            </select> --%>
                            ...
                            <select name="searchType">
                                <option value="all" <c:if test="${searchType == null || searchType == 'all'}">selected
                                    </c:if>>전체</option>
                                <option value="title" <c:if test="${searchType == 'title'}">selected</c:if>>제목</option>
                                <option value="content" <c:if test="${searchType == 'content'}">selected</c:if>>내용
                                </option>
                                <option value="writerNo" <c:if test="${searchType == 'writerNo'}">selected</c:if>>작성자번호
                                </option>
